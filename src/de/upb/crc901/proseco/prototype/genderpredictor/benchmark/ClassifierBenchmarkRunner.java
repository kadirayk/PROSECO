...
public class ClassifierBenchmarkRunner extends AbstractBenchmarkRunner {
	...
	public ClassifierBenchmarkRunner(final BenchmarkTask pTask, final GroundingRoutine pGroundingRoutine, final File taskTempFolder, final InstancesTuple tuple) {
		...
		int hashCode = 0;
		try {
			hashCode = new HashCodeBuilder().append(FileUtil.readFileAsString(instances.trainingData.getParent() + File.separator + "train.arff")).toHashCode();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void run() {
		try {
			...
			writeStats(usedClassifier, end - start, accuracy);
			log("DONE");
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	private void writeStats(Classifier algorithm, long runtime, double accuracy) {
		...
		try {
			...
			File dataTarget = new File(folder.getAbsolutePath() + File.separator + hashCodeOfTrainingData + ".arff");
			if (!dataTarget.exists()) {
				File dataSrc = new File(instances.trainingData.getParent() + File.separator + "train.arff");
				try {
			...
			try (FileWriter fw = new FileWriter(new File(MONITOR_OUTPUT) + File.separator +  "stats.csv", true)) {
				if (!folder.exists())
					folder.mkdirs();
				fw.write(hashCodeOfTrainingData + ", " + WekaUtil.getClassifierDescriptor(algorithm) + ", " + runtime + ", " + accuracy + "\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		} catch (Exception e) {
