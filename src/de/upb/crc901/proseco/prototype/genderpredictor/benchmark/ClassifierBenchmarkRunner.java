...
public class ClassifierBenchmarkRunner extends AbstractBenchmarkRunner {
	...
	public ClassifierBenchmarkRunner(final BenchmarkTask pTask, final GroundingRoutine pGroundingRoutine,
			final File taskTempFolder, final InstancesTuple tuple) {
	...
	private void computeFValue() {
		...
		if (new File(this.getTaskTempFolder().getAbsolutePath() + File.separator + CLASSIFIER_MODEL_FILE).exists()) {
			final ProcessBuilder pb = new ProcessBuilder(
					this.getTaskTempFolder().getAbsolutePath() + File.separator + "test.bat",
					this.instances.validationData.getAbsolutePath());
		...
		try (BufferedWriter bw = new BufferedWriter(
				new FileWriter(this.getTaskTempFolder() + File.separator + FVALUE_FILE))) {
			bw.write(f + "\n");
		} catch (final IOException e) {
			e.printStackTrace();
		}
