...
public class PrototypeBasedComposer {
	...
	public PrototypeBasedComposer(final String prototypeName, final String dataFilePath) {
		...
		try {
			...
			PerformanceLogger.logStart("executeFinalTest");
			this.executeFinalTest();
			PerformanceLogger.logEnd("executeFinalTest");
		...
		for (final File strategyFile : winningStrategy.listFiles()) {
			if (strategyFile.isFile()) {
				...
				try {
					...
					System.out.println("\nCould not move placeholder file " + strategyFile.getName() + " from " + strategyFile.getAbsolutePath() + " to "
							+ groundingFolderFile.getAbsolutePath());
					e.printStackTrace();
				}
			}
		}
	...
	/**
	 * Wait (busy waiting) for the processes to terminate that have been started to execute the different strategies.
	 *
	 */
